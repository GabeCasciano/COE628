...
int main(void) {
    ...
    //printf_function(NULL);  //for part 1 q.10
    pthread_mutex_unlock(&mutex1);
    pthread_mutex_destroy(&mutex1);//cleaning up
    
    //H: If all generator has finished fire signal for condition variable
    //I moved this here because it made more sense rather than increasing the size of the threads.
    pthread_mutex_lock(&mut);//lock mutex for cond signal, locked by calling thread to prevent undefined behavior.
    pthread_cond_signal(&cond);//wake prtiner;
    pthread_mutex_unlock(&mut);
    
    //E: Makes sure that print thread has finished before proceeding
    pthread_join(printer, NULL);
    
    pthread_mutex_destroy(&mut);//cleaning up
